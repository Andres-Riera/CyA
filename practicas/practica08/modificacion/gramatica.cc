...
Gramatica Gramatica::Chomsky() {
  ...
  char arranque{simbolo_arranque_};
  ...
  return Gramatica(terminals, non_terminals, productions, simbolo_arranque_);
}
...
bool Gramatica::EsLinealPorLaDerecha() {
  bool resultado{1};
  for (Produccion i : producciones_) {
    if (i.regla_.size() > 1) {
      for (int j = 0; j < i.regla_.size() - 1; j++) {
        if (IsIn(i.regla_[j], no_terminales_)) {
          resultado = 0;
        }
      }
    }
